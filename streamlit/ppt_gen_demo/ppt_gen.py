        ...
        if shape.has_text_frame:
            ...
            for paragraph in text_frame.paragraphs:
                ...
                if '###Slide-1-Description' in paragraph.text:
                    response_text = f"""
                    You are a marketing manager. You need to design a powerpoint for selling {product}. 
                    You are working on one slide with the subject: {ideas["Slide1Subject"]} and Description: {ideas["Slide1Description"]}.
                    The contant must within 30 words.
                    """
        ...
        if shape.has_text_frame:
            ...
            for paragraph in text_frame.paragraphs:
                ...
                if '###Slide-2-Description' in paragraph.text:
                    response_text = f"""
                    You are a marketing manager. You need to design a powerpoint for selling {product}. 
                    You are working on one slide with the subject: {ideas["Slide2Subject"]} and Description: {ideas["Slide2Description"]}.
                    The contant must within 50 words.
                    """
                    url = "http://insource-test-015.southeastasia.cloudapp.azure.com:8000/run"
                    payload = json.dumps({
                    "id": "1",
        ...
        if shape.has_text_frame:
            ...
            for paragraph in text_frame.paragraphs:
                if '###Slide-3-Subject' in paragraph.text:
                    paragraph.text = ideas["Slide3Subject"]
                if '###Slide-3-Description' in paragraph.text:
                    response_text = f"""
                    You are a marketing manager. You need to design a powerpoint for selling {product}. 
                    You are working on one slide with the subject: {ideas["Slide3Subject"]} and Description: {ideas["Slide3Description"]}.
                    The contant must within 50 words.
                    """
                    url = "http://insource-test-015.southeastasia.cloudapp.azure.com:8000/run"
                    payload = json.dumps({
...
def insource_idea(product):
    response_text = f"""
    You are a marketing manager. You need to design a powerpoint for selling the {product}
...
def main():  
    ...
    with col1:
        pptstyle = st.selectbox(
            "Choose your PowerPoint Style",
            ('ClockBlockDesign', 'GeometricColorBlock', 'Teccelerates'),
        )
    with col2:
